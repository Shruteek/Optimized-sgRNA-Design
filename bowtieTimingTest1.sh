...
startTime=$(date +%s)
bowtie -a -v 2 --verbose --large-index 101_R1_illumina_index --suppress 1,5,6,7 -f bowtie_test_guides.fasta -S Outputs/bowtieTimingTest1.sam
timePassed=$(expr $(date +%s) - $startTime)
echo $timePassed >> Outputs/bowtieTimingTest1.out