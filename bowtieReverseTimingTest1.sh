#!/bin/bash
#SBATCH --job-name=align_ctx_with_bowtie_reverse
#SBATCH -J download
#SBATCH --time=0:15:00
...
startTime=$(date +%s)
bowtie -a -f -v 2 --verbose bowtie_test_guides --suppress 1,5,6,7 101_R1_illumina.fasta -S Outputs/bowtieReverseTimingTest1.sam
timePassed=$(expr $(date +%s) - $startTime)
echo $timePassed >> Outputs/bowtieReverseTimingTest1.out